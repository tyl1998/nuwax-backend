...
docker run -d \
  ...
  -p "$HOST_COMPUTER_PROXY_PORT:$COMPUTER_PROXY_PORT" \
  "${VOLUME_ARGS[@]}" \
  ...
  -e PLATFORM_BASE_URL="${PLATFORM_BASE_URL:-}" \
  -e STORAGE_TYPE="${STORAGE_TYPE:-file}" \
